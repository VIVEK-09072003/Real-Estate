...
const AdminDashboard = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get('https://real-estate-rm3p.onrender.com/api/admin/users', {
  ...
  const handleBlockToggle = async (userId, currentStatus) => {
    ...
    if (window.confirm(`Are you sure you want to ${action} this user?`)) {
      try {
        await axios.patch(`https://real-estate-rm3p.onrender.com/api/admin/users/${userId}/block`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchUsers(); 
      } catch (err) {
  ...
  const handleApproveToggle = async (userId, currentApproval) => {
    ...
    if (window.confirm(`Do you want to ${action} this dealer to list properties?`)) {
      try {
        await axios.patch(`https://real-estate-rm3p.onrender.com/api/admin/users/${userId}/approve`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
        fetchUsers();
      } catch (err) {
